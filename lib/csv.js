...
module.exports = function(opts) {
	...
	if (fields.indexOf("PUMA") > -1) {
		fields.push("PUMAAREA");
	}

	// make a field for YEAR as a date...
	if (fields.indexOf("YEAR") > -1) {
		fields.push("YEAR_DATE");
	}

	// make a field for capturing hispanic + race
	if (fields.indexOf("RACE") > -1 && fields.indexOf("HISPAN") > -1) {
		fields.push("RACE_WITH_HISPANIC");
	}
	...
	rl.on('line', function(line) {
		...
		if (opts.full_pumas) {
			if (datum.PUMA) {
				...
				// add the puma area
				var pumaArea = 0;
				if (pumasJson[full_puma]) {
					pumaArea = pumasJson[full_puma]["area"];
				}
				datum.PUMAAREA = {
					val: pumaArea
				};
		...
		// custom fields
		var yearDate = datum.YEAR.val + "-01-01";
		datum.YEAR_DATE = {
			val: yearDate,
			label: '',
			valStr: yearDate
		...
		var race = datum.RACE_bucketed.val;
		var hispan = datum.HISPAN.val;
		var raceh = "Other";
